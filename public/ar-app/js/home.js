...
function addCount(delta) {
  if (state.goal === 0) { showToast('먼저 인원 수를 선택하세요!'); return; }
...
function updateHomeUI() {
  ...
  if (done) {
    ...
    btn.textContent = '📸 AR 인증샷 찍기!';
  } else {
    btn.disabled = tot === 0;
    btn.classList.add('locked');
    btn.textContent = tot > 0
      ? `🔒 ${tot - cur}명 더 전도하면 해금!`
      : '🔒 인원 수를 먼저 선택';
  }
}

// ── 홈 유저바 + 도감 버튼 삽입 (로그인 후) ────────────────
function injectHomeUserBar(user) {
  ...
  bar.innerHTML = `
    ...
    <button onclick="doLogout()" style="font-size:11px;color:rgba(255,255,255,.35);background:transparent;border:1px solid rgba(255,255,255,.15);border-radius:99px;padding:5px 11px;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">로그아웃</button>
  `;
  ...
  colBtn.style.cssText = "position:absolute;bottom:56px;right:20px;z-index:1;background:rgba(255,215,0,.12);border:1px solid rgba(255,215,0,.3);color:#FFD700;font-size:12px;border-radius:99px;padding:7px 14px;cursor:pointer;font-family:'Noto Sans KR',sans-serif;";
