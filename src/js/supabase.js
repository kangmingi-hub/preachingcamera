...
export async function saveToCollection(char) {
  try {
    const { data: ex } = await sb.from('collections').select('id,count')
      .eq('user_id', arUser.id).eq('character_id', char.id).maybeSingle();
    if (ex) {
      await sb.from('collections').update({ count: ex.count+1, last_obtained_at: new Date().toISOString() }).eq('id', ex.id);
      showToast(`📖 ${char.name} +1 (${ex.count+1}번째 획득!)`);
    } else {
      await sb.from('collections').insert({
        user_id: arUser.id, character_id: char.id, character_name: char.name,
